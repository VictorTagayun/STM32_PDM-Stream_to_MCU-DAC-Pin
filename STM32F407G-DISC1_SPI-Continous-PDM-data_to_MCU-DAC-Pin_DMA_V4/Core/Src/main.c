...
//	GPIOD->BSRR = (1 << (15 + 16)); // Reset
	...
	HAL_SPI_Receive_DMA(&hspi2, SPI_PDM_rx_data, 16);

	HAL_DAC_Start_DMA(&hdac, DAC_CHANNEL_1, (uint32_t*) PCM_DAC_outBuffer, 2, DAC_ALIGN_12B_R); // 4000bytes x 8bit/bytes = 32,000 bit / 64 decimate = 500 int of data
...
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi)
{
   ...
   */
  	  ...
  	  GPIOD->BSRR = (1 << 15); // Set
  	  // 2000bytes x 8bit/bytes = 16,000 bit / 64 decimate = 250 int of data
  	  PDM_Filter(&SPI_PDM_rx_data[8], &PCM_outBuffer[1], &PDM1_filter_handler); // 2000bytes x 8bit/bytes = 16,000 bit / 64 decimate = 250 int of data

  	  PCM_DAC_outBuffer[1] = (PCM_outBuffer[1] >> 4) + 2048; // convert to 12bit DC offset of DAC data
